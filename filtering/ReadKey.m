...
% Xiangrui Li, 06/2009, make it compatible to PTB 2 and 3
% Xiangrui Li, 05/2011, add CharAvail to take case of fOPR problem

function [key, secs] = ReadKey (KEYS)
    persistent isPTB2 useCharAvail;
    if isempty(isPTB2)
        os=Screen('Computer'); % rely on PTB2 has no osx field
        isPTB2= ~isfield(os,'osx');
        useCharAvail=true;
        try, CharAvail; catch foo; useCharAvail=false; end %#ok
    end
    ...
    elseif useCharAvail && CharAvail % workaround for fORP USB
        ch=GetChar;
        if any(strcmp(ch,KEYS)), key=ch; end
    end
end
...
elseif islogical(in)
    ...
    for i=1:length(in), ind=strmatch(in{i},kk,'exact'); out(i)=ind(1); end %#ok
end
return;
