% for biosemi scripts  avoid popping up windows  default: Ok is clicked
...
function [EEG, com] = pop_rmbase_new( EEG, timerange, pointrange);
...
if nargin < 2 & EEG(1).trials > 1
	...
	result       = inistr; % inputdlg2( promptstr, 'Epoch baseline removal -- pop_rmbase()', ...
                               %   1,  inistr, 'pop_rmbase');
...
%-----------------------------------------%
disp('Pointrange for rmbase: ');
fprintf('.');fprintf('.');fprintf(num2str(pointrange(1,1)));
for i=1:floor(size(pointrange,2)/5)
        if mod(i,16)==0
            fprintf('\n');
        end
        fprintf('.');fprintf('.');fprintf(num2str(pointrange(1,i*5)));
end
fprintf('.');fprintf('.');fprintf(num2str(pointrange(1,end)));
fprintf('\n');
% disp(pointrange);
%-----------------------------------------%
